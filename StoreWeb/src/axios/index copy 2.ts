...
/**
 * Blob / ArrayBuffer 二进制上传PDF转Excel
 * @param pdfBlob PDF二进制Blob
 * @returns 文件访问地址
 */
async function uploadPdfByBlob(pdfBlob: Blob): Promise<string> {
  const userStore = useUserStoreWithOut()
  const token = userStore.getToken
  const tokenKey = userStore.getTokenKey
  ...
  const file = new File([pdfBlob], "temp.pdf", { type: "application/pdf" });
  ...
  try {
    const res = await fetch(`${baseUrl}/api/pdf/PdfFileToExcel`, {
      method: "POST",
      body: formData,
      headers
    });

    const data: IResponse<{ path: string }> = await res.json();
    if (!data.success) {
      ElMessage.error(data.msg || 'PDF转换失败')
      throw new Error(data.msg)
    }
    console.log("生成文件地址：", data.data.path);
    return data.data.path;
  } catch (err) {
    ElMessage.error('PDF上传转换异常')
    console.error('uploadPdfByBlob error', err)
    throw err
  }
}
