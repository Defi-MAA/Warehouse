<template>
  <ContentWrap>
    ...
    <el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
      ...
      <el-table-column align="center" prop="ayymm" label="会计期间" :sortable="'custom'" width="150px">
      </el-table-column>
      <el-table-column align="center" prop="adesc" label="说明" :sortable="'custom'" width="150px">
      </el-table-column>
      <el-table-column align="center" prop="abdat" label="开始日期" :sortable="'custom'" width="170px">
      </el-table-column>
      <el-table-column align="center" prop="aedat" label="结束日期" :sortable="'custom'" width="170px">
      </el-table-column>
      <el-table-column align="center" prop="astat" label="状态" :sortable="'custom'" width="150px">
        <template #default="{ row }">
          {{ row.astat == 1 ? '关账' : '开账' }}
        </template>
      </el-table-column>
      <el-table-column align="center" prop="amemo" label="备注" :sortable="'custom'" width="150px">
  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    <el-form :model="form" label-width="auto" ref="ruleFormRef" :rules="rules">
      <el-row :gutter="10">
        <el-col :span="12">
          <el-form-item label="会计期间" prop="ayymm">
            <el-input readonly v-model="form.ayymm"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="说明" prop="adesc">
            <el-input v-model="form.adesc"></el-input>

          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="开始日期" prop="abdat">

            <el-date-picker v-model="form.abdat" type="date" value-format="YYYY-MM-DD" placeholder="选择日期">
            </el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="结束日期" prop="aedat">

            <el-date-picker v-model="form.aedat" type="date" value-format="YYYY-MM-DD" placeholder="选择日期">
            </el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="状态" prop="astat">
            <el-checkbox v-model="form.astat" >关账</el-checkbox>
            <!-- <el-option label="开账" value="0">
              </el-option>
              <el-option label="关账" value="1">
              </el-option>
            </el-select> -->
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="备注" prop="amemo">
            <el-input v-model="form.amemo"></el-input>
          </el-form-item>
        </el-col>
...
const sorts = ref({ order: 'descending', prop: 'ayymm' })
const getSorts = () => {
  let order = "";
...
const getList = async () => {
  ...
  const res = await request.post({ url: '/api/prd/selectPrdPage', data })
...
const editData = async (row: any) => {
  ...
  const res = await request.post({ url: '/api/prd/getPrd', data: { ayymm: row.ayymm } })
  form.value = res.data || {};
...
const delData = async (row: any | null) => {
  if (row == null) {
    ids.value = checkData.value.map((v: any) => v.ayymm) || []
  } else {
    ids.value = [row.ayymm]
  }
    ...
    if (valid) {
      ...
      if (formData) {
        if (actionType.value != 'edit')
          formData.ayymm = formData.abdat?.substring(0, 7) || '';
        saveLoading.value = true
        const res = await request.post({
          url: actionType.value === 'edit' ? '/api/prd/updatePrd' : '/api/prd/insertPrd',
          data: formData
        }).catch(() => {
