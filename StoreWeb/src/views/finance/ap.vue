<template>
  <ContentWrap>
    ...
    <div class="mb-10px" style="margin-top: 10px;">
      <BaseButton type="primary" :icon="Money" @click="settleData()">付款</BaseButton>
    ...
    <el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
      ...
      <el-table-column prop="vendorname" label="供应商" show-overflow-tooltip sortable="custom" width="150">
      </el-table-column>
      <el-table-column prop="amount" label="应付总额" show-overflow-tooltip sortable="custom" width="120">
      </el-table-column>
      <el-table-column prop="paidamount" label="已付金额" show-overflow-tooltip sortable="custom" width="120">
      </el-table-column>
      <el-table-column prop="unpaidamount" label="未付金额" show-overflow-tooltip sortable="custom" width="120">
      </el-table-column>
  ...
  <Dialog v-model="dialogVisible" width="850px" max-height="400px" title="付款">
    ...
    <el-row :gutter="10">
      <el-table :data="settleList" :header-cell-style="{ color: '#000000' }" stripe :border="true" show-summary
        ...
        <el-table-column prop="vendorname" label="供应商" show-overflow-tooltip width="150">
        </el-table-column>
        <el-table-column prop="amount" label="应付总额" show-overflow-tooltip width="120">
        </el-table-column>

        <el-table-column prop="paidamount" label="已付金额" show-overflow-tooltip sortable="custom" width="120">
        </el-table-column>
        <el-table-column prop="unpaidamount" label="实付金额" show-overflow-tooltip width="190">
          <template #default="{ row }">
    ...
    <template #footer>
      <el-row :gutter="10">
        <el-form :model="form" :rules="rules" ref="ruleFormRef" label-width="120px">
          <el-form-item label="付款方式" prop="settlementdate">
  ...
  <Dialog v-model="dlgPayed" width="80%" maxHeight="500px" title="付款单">
    <el-row :gutter="10">
      <el-col :span="12">
        <el-table :data="payList" :header-cell-style="{ color: '#000000' }" ref="payListRef" @row-click="rowClick"
          stripe :border="true" height="390px">
          <el-table-column prop="payment_no" label="付款单号" show-overflow-tooltip width="150">

          </el-table-column>
          <el-table-column prop="amount" label="付款总额" show-overflow-tooltip width="120">

          </el-table-column>
          <el-table-column prop="payname" label="付款方式" show-overflow-tooltip width="120">

          </el-table-column>
          <el-table-column label="操作" min-width="180" fixed="right">
            <template #default="{ row }">
              <el-button type="primary" link @click="viewReportBill(row.payment_no)" style="margin-right: 10px;">
                打印
              </el-button>

              <el-button type="primary" link @click="cancelPayment(row)" style="margin-right: 10px;">
                取消
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-col :span="12">
        <el-table :data="payList1" :header-cell-style="{ color: '#000000' }" stripe :border="true" height="390px">
          <el-table-column prop="payment_date" label="日期" show-overflow-tooltip width="170">

          </el-table-column>
          <el-table-column prop="source_tcode" label="单号" show-overflow-tooltip width="150">

          </el-table-column>
          <el-table-column prop="amount" label="付款金额" show-overflow-tooltip width="150">
...
const getList = async () => {
  ...
  const res = await request.post({ url: '/api/apdetail/selectApdetailPage', data })
...
const editData = async (row: any) => {
  ...
  const res = await request.post({ url: '/api/apdetail/getApdetail', data: { id: row.id } })
  form.value = res.data || {};
...
ElMessageBox.confirm('确定冲销选中的数据吗？', '冲销确认', {
  ...
  checkData.value.forEach(async (item: any) => {
    request.post({ url: '/api/finance/voidPayment', data: { id: item.id } }).then(res => {
      fnum++
      if (fnum >= n) {
...
const viewPayed = async (row: any) => {
  ...
  payList.value = []
  payList1.value = []
  const res = await request.post({ url: '/api/finance/getPaymentList', data: { id: row.id } })
  if (res && res.code === 0) {
...
const rowClick = async (row: any) => {
  const res1 = await request.post({ url: '/api/finance/getPaymentDetailList', data: { no: row.payment_no } })
  if (res1 && res1.code === 0) {
...
const viewDetail = async (row: any) => {
  ...
  const res = await request.post({ url: '/api/finance/getPaymentDetail', data: { id: row.id } })
  if (res && res.code === 0) {
...
const cancelPayment = async (row: any) => {
  ElMessageBox.confirm('确定取消选中的付款吗？', '取消确认', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    const res = await request.post({ url: '/api/finance/cancelPayment', data: { no: row.payment_no } })
    if (res && res.code === 0) {
