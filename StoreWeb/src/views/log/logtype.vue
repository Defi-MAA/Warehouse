...
const getList = async () => {
    ...
    const res = await request.post({ url: '/api/eventlogs/selectEventlogsPage', data })
...
const editData = async (row: any) => {
    ...
    const res = await request.post({ url: '/api/eventlogs/getEventlogs', data: { id: row.id } })
    form.value = res.data || {};
...
const delData = async (row: any | null) => {
    if (row == null) {
        ids.value = checkData.value.map((v: any) => v.id) || []
    } else {
        ids.value = [row.id]
    }
    if (ids.value.length == 0) {
        ...
        ElMessageBox.confirm('确定删除选中的数据吗？', '删除确认', {
            ...
            const res = await request.post({
                url: '/api/eventlogs/deleteEventlogss',
                data: {
                    ids: ids.value
                }
            }).catch(() => {
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            if (formData) {
                ...
                const res = await request.post({
                    url: actionType.value === 'edit' ? '/api/eventlogs/updateEventlogs' : '/api/eventlogs/insertEventlogs',
                    data: formData
                }).catch(() => {
...
<template>
    <ContentWrap>
        ...
        <el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
            ...
            <el-table-column prop="idtime" label="idtime" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="url" label="url" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="descript" label="descript" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="userid" label="userid" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="username" label="username" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="clerkweb" label="clerkweb" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            <el-table-column prop="ip" label="ip" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>

            <el-table-column label="操作" min-width="120">
                <template #default="{ row }">
                    <el-button type="primary" :icon="Edit" @click="editData(row)">
                        编辑
                    </el-button>
                    <el-button type="danger" :icon="Delete" @click="delData(row)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    <Dialog v-model="dialogVisible" :title="dialogTitle">
        <el-form :model="form" label-width="auto" ref="ruleFormRef" style="max-width: 600px" :rules="rules">
            <el-row :gutter="10">
                <el-col :span="6">
                    <el-form-item label="idtime" prop="idtime">
                        <el-date-picker v-model="form.idtime" type="date" format="yyyy-MM-dd"
                ...
                <el-col :span="6">
                    <el-form-item label="clerkweb" prop="clerkweb">
                        <el-input v-model="form.clerkweb" />
                    </el-form-item>
                </el-col>
                <el-col :span="6">
