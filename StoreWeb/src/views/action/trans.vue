  ...
  <ContentWrap>
    <div>
      ...
      <div style="border-bottom: 1px solid #e4e7ed;">
        <Search show-expand expand-field="ttype" :schema="searchSchema" @reset="setSearchParams"
      ...
      <el-table v-loading="listLoading" :data="list" @sort-change="sort" border fit highlight-current-row
        style="width: 100%" :height="tableHeight - 50" @selection-change="setCheck" id="out-table"
        :header-cell-style="{ color: '#000000' }" ref="tableRef" :default-sort="{
          prop: 'tcode',
          order: 'descending'
        }">
        ...
        <el-table-column  prop="custname" label="供应商" width="100px">
				</el-table-column>
				<el-table-column  prop="wrname" label="仓库" width="100px">
				</el-table-column>
				<el-table-column  prop="deptname" label="部门" width="100px">
				</el-table-column>
				<el-table-column  prop="salename" label="销售单位" width="100px">
				</el-table-column>
      ...
      <el-container style="width: 100%;">
        <el-header>
          <el-row>
            ...
            <div style="display:inline-block;">
              <label class="radio-label" style="padding-left:10px;">结算方式</label>
              <el-select v-model="form.apitem" style="width: 120px; " placeholder="请选择">
            ...
            <div style="display:inline-block;">
              ...
              <el-date-picker style="width: 140px; " v-model="form.pricedate" type="date" value-format="YYYY-MM-DD"
                placeholder="选择日期">
              </el-date-picker>
            </div>
          </el-row>
          <el-row>
            ...
            <div style="display:inline-block;">
              <label class="radio-label" style="padding-left:10px;"></label>
              <el-checkbox v-model="form.fistmp" style="padding-left:10px;">退货标志</el-checkbox>
            </div>
            <div style="display:inline-block;">
              <label class="radio-label" style="padding-left:10px;">供应商</label>
              <el-select style="width: 120px; " v-model="form.vndr" placeholder="请选择">
                <el-option v-for="(item, index) in custlist" :key="index" :label="item.name" :value="item.code">
                </el-option>
              </el-select>
            </div>
            <div style="display:inline-block;">
        ...
        <el-main>
          <el-table v-loading="listLoading1" :data="items" :border="true" fit highlight-current-row style="width: 100%"
          height="330px" id="out-table1" ref="gitems" @current-change="rowChage"
            ...
            <el-table-column prop="lot" label="批次" width="150px">
              <template #default="{ row }">
                <!-- <el-input v-model="row.lot" type="text" @focus="showLot(row)" placeholder="" /> -->
                <el-input v-model="row.lot" type="text" placeholder="" />
              </template>
            </el-table-column>
            <el-table-column prop="qty" label="数量" width="90px">
              <template #default="{ row }">
                <el-input v-model="row.qty" @change="setAmt(row)" type="number" @focus="focus($event)" placeholder="" />
              </template>
            </el-table-column>
            ...
            <el-table-column prop="amt" label="金额" width="120px">
              <template #default="{ row }">
                <el-input v-model="row.amt" readonly type="number" @focus="focus($event)" placeholder="" />
              </template>
            </el-table-column>
            <el-table-column prop="expdate" label="保质期" width="180px">
              <template #default="{ row }">
                <el-date-picker v-model="row.expdate" value-format="YYYY-MM-DD" style="width: 140px;" type="date"
                  placeholder="选择日期">
                </el-date-picker>
              </template>
            </el-table-column>
        ...
        <el-footer style="height: 140px;">
          ...
          <el-row>
            <div style="display:inline-block;">
              ...
              <el-select style="width: 120px; " v-model="form.man1" placeholder="请选择">
                <el-option v-for="(item, index) in userlist" :key="index" :label="item.name" :value="item.code">
                </el-option>
              </el-select>
            </div>
            <div style="display:inline-block;">
          ...
          <el-row style="text-align: right;margin-top:10px;display: flex; justify-content: flex-end;">
            <el-button type="primary" @click="doPrint(form)" :disabled="!isedit" :icon="Printer">打印</el-button>
    ...
    <Dialog v-model="dlgLot" width="70%" :show-close="false" :close-on-click-modal="false" top="50px">
      <div slot="title" style="display: flex;height: 40px; line-height: 40px;background-color: #409EFF;margin: -20px;">
        ...
        <div style="flex: 1;text-align: right;">
          <i class="el-icon-close closeico"
            style="color: #FFFFFF; font-size: 20px;line-height: 40px;margin-right: 20px;" @click="dlgLot = false"></i>
        </div>
      </div>
...
const delGoods = (row: ItemData) => {
  items.value = items.value.filter((item: ItemData) => item.index != row.index)
}
...
const passTrans = () => {
  ...
  ElMessageBox.confirm('是否确定过账单据?', '提示', {
    ...
    checkList.value.forEach(o => {
      let url = '/api/tran/passTran'
      if (o.ttype == 'IC') {
        url = '/api/tran/passTranIC'
      }
      request.post({
        url: url, data: {
          tcode: o.tcode
