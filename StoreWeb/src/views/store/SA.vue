            ...
            <el-container style="width: 100%;">
                <el-header>
                    ...
                    <el-row>
                        ...
                        <div style="display:inline-block;">
                            <label class="radio-label" style="padding-left:10px;">销售单位</label>
                ...
                <el-main>
                    <el-table v-loading="listLoading1" :data="items" :border="true" fit highlight-current-row
                        ...
                        <el-table-column prop="curprice" label="销售单价" width="120px">
                            <template #default="{ row }">
                                <el-input v-model="row.curprice" @change="setAmt(row)" type="number"
                                    @focus="focus($event)" placeholder="" />
                            </template>
                        </el-table-column>
...
const vendorlist = ref<OptionItem[]>([])
...
const initData = () => {
    ...
    request.post({ url: '/api/config/getAllData', data: {} }).then(res => {
        ...
        vendorlist.value = res.data.sales
    }).catch(error => { })
}
...
const setAmt = (row: ItemData) => {
    if (row.qty && row.price) {
        ...
        row.factamt = row.qty * row.curprice
    } else {
        row.amt = 0
        row.factamt = 0
    }
