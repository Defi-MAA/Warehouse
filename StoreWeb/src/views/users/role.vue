...
const getList = async () => {
    ...
    const res = await request.post({ url: '/api/users/selectUrolePage', data })
...
const editData = async (row: any) => {
    ...
    const res = await request.post({ url: '/api/users/getUrole', data: { code: row.code } })
    form.value = res.data || {};
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            if (formData) {
                ...
                const res = await request.post({
                    url: actionType.value === 'edit' ? '/api/users/updateUrole' : '/api/users/insertUrole',
                    data: formData
                }).catch(() => {
...
<template>
    <ContentWrap>
        ...
        <el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
            ...
            <el-table-column prop="name" label="名称" show-overflow-tooltip sortable="custom" width="180">
            </el-table-column>
            ...
            <el-row :gutter="10">
                ...
                <el-col :span="24">
                    <el-form-item label="备注" prop="memo">
                        <el-input v-model="form.memo" maxlength="255" />
                    </el-form-item>
                </el-col>
