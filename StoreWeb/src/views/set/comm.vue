<template>
	<ContentWrap>
		...
		<el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
			...
			<el-table-column align="center" prop="code" label="代码" :sortable="'custom'" width="150px">
			</el-table-column>
			<el-table-column align="center" prop="name" label="内容" show-overflow-tooltip :sortable="'custom'" width="150px">
			</el-table-column>
			<el-table-column align="center" prop="memo" label="备注" show-overflow-tooltip :sortable="'custom'" width="150px">
			</el-table-column>
...
const getList = async () => {
	...
	const res = await request.post({ url: '/api/phr/selectPhrPage', data })
...
const editData = async (row: any) => {
	...
	const res = await request.post({ url: '/api/phr/getPhr', data: { code: row.code } })
	form.value = res.data || {};
	...
	await formEl.validate(async (valid) => {
		if (valid) {
			...
			if (formData) {
				...
				const res = await request.post({
					url: actionType.value === 'edit' ? '/api/phr/updatePhr' : '/api/phr/insertPhr',
					data: formData
				}).catch(() => {
