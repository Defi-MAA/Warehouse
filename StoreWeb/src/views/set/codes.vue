<template>
	<ContentWrap>
		...
		<el-table :data="tableData" @sort-change="handleSortChange" :default-sort="{
			...
			<el-table-column prop="classname" label="分类" show-overflow-tooltip sortable="custom" width="150">
			</el-table-column>
			<el-table-column prop="code" label="代码" show-overflow-tooltip sortable="custom" width="150">
			</el-table-column>
			<el-table-column prop="name" label="名称" show-overflow-tooltip sortable="custom" width="150">
			</el-table-column>
			<el-table-column prop="value" label="值" show-overflow-tooltip sortable="custom" width="150">
			</el-table-column>
			<el-table-column prop="notes" label="备注" show-overflow-tooltip sortable="custom" width="180">
			</el-table-column>
			
			<el-table-column prop="flag" label="状态" show-overflow-tooltip sortable="custom" width="100">
			...
			<el-row :gutter="10">
				<el-col :span="8">
					<el-form-item label="分类" prop="class">
						<el-select v-model="form.tclass" placeholder="请选择">
							<el-option v-for="item in typeids" :label="item.name" :value="item.code">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item label="代码" prop="code">
						<el-input v-model="form.code" maxlength="8" :readonly="actionType === 'edit'" />
					</el-form-item>
				</el-col>
				<el-col :span="8">
					<el-form-item label="状态" prop="flag">
						<el-checkbox v-model="form.flag"></el-checkbox>
					</el-form-item>
				</el-col>
				<el-col :span="8">
				...
				<el-col :span="8">
					<el-form-item label="备注" prop="notes">
						<el-input v-model="form.notes" maxlength="30" />
					</el-form-item>
				</el-col>
				<el-col :span="8">
...
const initData = async () => {
	let res = await request.post({ url: '/api/codes/getTClass', data: {} })
...
const getList = async () => {
	...
	const res = await request.post({ url: '/api/codes/selectCodesPage', data })
...
const editData = async (row: any) => {
	...
	const res = await request.post({ url: '/api/codes/getCodes', data: { code: row.code } })
	form.value = res.data || {};
	...
	await formEl.validate(async (valid) => {
		if (valid) {
			...
			if (formData) {
				...
				const res = await request.post({
					url: actionType.value === 'edit' ? '/api/codes/updateCodes' : '/api/codes/insertCodes',
					data: formData
				}).catch(() => {
