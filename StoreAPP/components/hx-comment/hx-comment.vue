		...
		<view class="hx-comment_main-box">
			...
			<view class="lists" v-if="listData.length > 0">
				<block v-for="(item, index_) in listData" :key="index_">
					<view class="item">
						<view class="icon"><image :src="item.avatar" mode="widthFix" style="width:100%" /></view>
						<view class="info">
							<view class="name-time">
								<text class="name">{{ item.nickname }}</text>
								<text class="time">{{ item.idtime }}</text>
							</view>
							<view class="stars">
								<text>评分</text> <uni-rate size="14" :value="item.xing"></uni-rate>
							</view>
							<view class="evaluate-content">
								<text>{{ item.content || '用户暂未评价' }}</text>
								<view class="imgs" v-if="item.srcs">
									<block v-for="(imgurl, index) in item.srcs" :key="index">
