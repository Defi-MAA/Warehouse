<template>
	<view>
		...
		<view class="flex solid-bottom  justify-start text-bold bg-gray">
			...
			<view class=" padding-sm  solid-right text-center" style="width: 15%;">申购数量</view>
			<view class=" padding-sm  solid-right text-center" style="width: 15%;">实购数量</view>
			<view class=" padding-sm  solid-right text-center" style="width: 20%;">单价</view>
			<view class=" padding-sm  solid-right text-center" style="width: 20%;">金额</view>
<!-- 			<view class=" padding-sm  solid-right text-center" style="width: 10%;"></view> -->
		</view>
		<view class=" flex solid-bottom justify-start " v-for="(item,index) in items">
			...
			<view class=" padding-sm  solid-right text-center bg-white" style="width: 20%;">
				<input v-model="item.Taxprice"></input>
			</view>
			<view class=" padding-sm  solid-right text-center bg-white" style="width: 20%;">
				<text>{{(item.rfquantity*item.Taxprice).toFixed(2)}}</text>
			</view>
			<!-- <view class=" padding-sm  text-center" style="width: 10%;">
				<button class=" cu-btn bg-red  round sm" style="font-size: 4px;width: 40px;"
			...
			<view class="padding flex flex-direction margin-top">
				<button v-if="info.status=='已确定'" class="cu-btn bg-blue margin-tb-sm lg" @click="submit">
					签名验收
					</button>
			</view>
		</form>
	...
	export default {
		...
		methods: {
			...
			async ConfigSign(sign) {
				this.$api.loading(true);
				let obj = await this.$api.ajax('/api/store/PassSign', {
					sign: sign,
					id: this.info.id,
					items:this.items
				});
